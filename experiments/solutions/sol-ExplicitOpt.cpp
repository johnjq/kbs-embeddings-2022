...
namespace frechet_ {
    ...
    std::vector<std::vector<int>> compute_embedding(std::vector<std::vector<int>>& adj) {
        ...
        int size1 = 0, size2 = 0;
        for (int v : positive) size1 += subtree_size[v];
        for (int v : negative) size2 += subtree_size[v];
        if (size2 <= size1) {
            vector<int> new_positive;
            for (int v : positive) {
                const int new_size1 = size1 - subtree_size[v];
                const int new_size2 = size2 + subtree_size[v];
                if (new_size2 <= new_size1) {
                    negative.push_back(v);
                    size1 = new_size1;
                    size2 = new_size2;
                } else {
